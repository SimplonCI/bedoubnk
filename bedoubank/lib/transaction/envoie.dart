...
class EnvoiePage extends StatefulWidget {
  @override
  _EnvoiePageState createState() => _EnvoiePageState();
}

class _EnvoiePageState extends State<EnvoiePage> {
  ...
  Widget build(BuildContext context) {
    ...
    String url = Global().env.apiBaseUrl+'/transfert.php';
    ...
    Future<List> _retrait() async{
      ...
      final response = await http.post(
          url,
          body: {
            'numeroEmetteur' : numeroEmetteur,
            'password': password ,
            'numeroRecepteur': numeroRecepteur,
            'montant': montant,
            'email': email ,
            'id' :  iduser
    ...
    Future<void> _confirmPopup(String title) async {
      return showDialog<String>(
        ...
        builder: (BuildContext context) => AlertDialog(
          ...
          content: Text(
            'Voullez vraiment effectuer un transfert  de ${title} vers ${beneficiaireController.text}',
          ),
          actions: <Widget>[
            FlatButton(
              child: Text('Annuler'),
    ...
    return Scaffold(
      ...
      body: Center(
        child: ListView(
            ...
            children: <Widget>[
              Text(
                "Formulaire d' envoie d'argent".toUpperCase(),
